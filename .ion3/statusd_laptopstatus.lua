...
function open_single( file, mode )
   local fdo = io.open( file, mode )
   if fdo then return fdo, file end
end
...
local function get_battery()
  ...
  if pcall(function ()
       local status
       local file=open_single( "/proc/acpi/battery/BAT0/info", "r")
       local infocontents = file:read("*all")
       file:close();
 
       local file=open_single( "/proc/acpi/battery/BAT0/state", "r")
     ...
     return { percent="n/a", timeleft="n/a", percenthint=percenthint, timelefthint=timelefthint} 
...
local function update_laptopstatus ()
  ...
  if statusd ~= nil then
    ...
    io.stdout:write("CPU: "..cpu.speed.." || BATT: "..battery.percent.." "..battery.timeleft.."\n")
    io.stdout:write("Temp: "..thermal.temperature.."\n")
  end
end
