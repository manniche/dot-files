...
function autoscreen_on()
   local f = io.popen('/home/stm/local/bin/dvi-screen -c', 'r')
   if not f then
      return
   end
   local s = f:read('*a')
...
function autoscreen_off()
   local f = io.popen('/home/stm/local/bin/dvi-screen.py -d', 'r')
   if not f then
      return
   end
   local s = f:read('*a')
...
defbindings("WMPlex.toplevel", {
               ...
               bdoc("Run a terminal emulator."),
               kpress(META.."F2", "ioncore.exec_on(_, ':ipython')"),

               bdoc("Query for command line to execute."),
               kpress(ALTMETA.."F3", "mod_query.query_exec(_)"),

               -- bdoc("Query for command line to execute or goto (app.lua)."),
               -- kpress(ALTMETA.."F3", "app.byclass(_)"),

               bdoc("Query for Lua code to execute."),
               kpress(META.."F3", "mod_query.query_lua(_)"),

               bdoc("Query for program line to execute or goto (via app.lua)."),
               kpress(ALTMETA.."F3", "mod_query.query_exec(_)"),
               ...
               bdoc("Go to or open emacs (via app.lua)."),
               kpress(ALTMETA.."F5", "app.byclass('emacs', 'Emacs')"),

               bdoc("Query for file to view."),
               kpress(ALTMETA.."F6",
                      "mod_query.query_runfile(_, 'run-mailcap --action=view')"),

               bdoc("Open the gtd system."),
               kpress(ALTMETA.."F7", "ioncore.exec_on(_, 'emacs -f gtd')"),
...
defbindings("WFrame.toplevel", {
               ...
               submap(META.."X", {
                         bdoc( "Emacs like splitting commands"),
                         kpress("2", "WIonWS.split_at(_, _sub, 'bottom', true)"),
                         kpress("3", "WIonWS.split_at(_, _sub, 'right', true)"),
                         kpress("0", "WIonWS.unsplit_at(_, _sub)"),
                      }),
